...
class Horse:
    ...
    def update_checkpoint(self):
        ...
        if dist < self.track.checkpoint_radius and self.checkpoint_reached_time == 0:
            ...
            if self.current_checkpoint_index == 0:
                print(f"{self.name} passed START checkpoint!")
            elif self.current_checkpoint_index == self.total_checkpoints - 1:
                print(f"{self.name} reached FINISH checkpoint! Race complete!")
                self.mark_finished()
                if self.ranking_manager:
                    self.ranking_manager.finish_race(self.horse_id)
                return
            else:
                print(f"{self.name} reached checkpoint {self.current_checkpoint_index}")
    ...
    def mark_finished(self):
        ...
        self.target = None
        self.current_path = []
        print(f"{self.name} has finished the race and stopped!")
    ...
    def draw(self, screen):
        if self.show_path and len(self.path_history) > 1:
            for i in range(0, len(self.path_history) - 1, 2):
        ...
        pygame.draw.polygon(screen, COLORS['BLACK'], points, 1)